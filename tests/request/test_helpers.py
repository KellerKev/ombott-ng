import pytest
from ombott_ng.request_pkg.helpers import FormsDict, CookieDict
...
def test_formsdict():
    ...
    assert fdict.unknown_key is None

    cpy = fdict.copy()

    assert cpy == fdict
    assert cpy.foo == 'foo'
    assert cpy.unknown_key is None
