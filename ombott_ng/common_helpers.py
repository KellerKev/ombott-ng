...
# ------------------[ context-safe props ] -------------------
# Per-request request/response state is held in a contextvars.ContextVar rather
# than threading.local. ContextVars are isolated PER THREAD under a sync/WSGI
# server (each thread has its own context -> behaviour-equivalent to
# threading.local) AND PER TASK under an async/ASGI server (each request task
# copies the context), which is what makes the global `request`/`response`
# singletons safe for concurrent async requests on a single event loop.
def ts_props(*props, store_name=None):
    def wrapper(cls):
        # one ContextVar per decorated class, created once at decoration time
        ctx_var = contextvars.ContextVar(store_name or ('%s.ts_props' % cls.__name__))

        cls_init = cls.__init__

        def get_store():
            store = ctx_var.get(None)
            if store is None:
                store = {}
                ctx_var.set(store)
            return store
        ...
        def init_wrapper(self, *a, **kw):
            ctx_var.set({k: None for k in props})
            cls_init(self, *a, **kw)

        def make_prop(k):
            def fget(s):
                return get_store().get(k)

            def fset(s, v):
                get_store()[k] = v

            def fdel(s):
                get_store().pop(k, None)
            doc = 'Context-local property: %s' % k
            return property(fget, fset, fdel, doc)

        cls.__init__ = init_wrapper
        [setattr(cls, p, make_prop(p)) for p in props]
        cls._ts_ctxvar = ctx_var
        return cls
    return wrapper
...
class HeaderDict(DictMixin):
    # backing dict is held in a per-instance ContextVar so the singleton
    # `response.headers` (whose backing is rebound per request via
    # `headers.dict = response._headers`) is isolated per thread (WSGI) and per
    # task (ASGI). See ts_props above.
    __slots__ = ('_cv',)

    dict = property(
        (lambda s: s._cv.get()),
        (lambda s, v: s._cv.set(v)),
    )

    def __init__(self, *a, **kw):
        self._cv = contextvars.ContextVar('ombott.headerdict')
        self._cv.set(dict(*a, **kw))
    ...
    def setdefault(self, key, value):
        return self.dict.setdefault(key, _hval(value) if not isinstance(value, list) else value)
